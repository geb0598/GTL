...
BatchLines::BatchLines()
{
	...
	Primitive.Vertexbuffer = Renderer.CreateVertexBuffer(
		Vertices.data(), Primitive.NumVertices * sizeof(FVertex));
	Primitive.Location = FVector(0, 0, 0);
	Primitive.Rotation = FVector(0, 0, 0);
	Primitive.Scale = FVector(1, 1, 1);
}

void BatchLines::UpdateBoundingBoxTransformBy(FBoundingBox boundingBoxInfo)
{
	Primitive.Scale = boundingBoxInfo.max - boundingBoxInfo.min;
	Primitive.Location = boundingBoxInfo.min;
}

void BatchLines::RenderBatchLines()
{
	URenderer& Renderer = URenderer::GetInstance();

	// to do: 아래 함수를 batch에 맞게 수정해야 함.
	Renderer.RenderPrimitiveIndexed(Primitive, Primitive.RenderState);
}

void BatchLines::AddWorldGridVerticesAndConstData()
{
...
void BatchLines::AddBoundingBoxVertices()
{
