...
핵심 목표는 **Wavefront OBJ 파일을 엔진의 자체 형식으로 변환하는 데이터 파이프라인을 구축**하고, **정적 메시(StaticMesh)를 씬에 배치하여 렌더링**하는 것이었습니다.  
또한, **다중 뷰포트와 개선된 에디터 UI**를 통해 작업 효율성을 극대화하고자 했습니다.  

본 문서는 이번 주에 구현된 주요 기능들을 **Editor & Rendering**과 **Engine Core** 두 가지 관점으로 나누어, 각 기능의 구현 목표(What), 설계 방식(How), 그리고 설계 의도(Why)를 상세히 설명합니다.  
...
* 단일 Perspective 뷰 → **Top, Front, Side 뷰 포함한 4분할 뷰포트 환경** 구현.
* 사용자는 **스플리터 드래그**로 뷰포트 크기 동적 조절 가능.
...
4주차 개발을 통해 **외부 3D 데이터를 엔진으로 가져와 렌더링하는 리소스 파이프라인**을 완성했습니다.

* **Raw → Cooked Data 변환**
* **Asset 관리 시스템 구축**
* **사용자 친화적 에디터 UI & 다중 뷰포트**

특히 **Parser, Serializer, Iterator**와 같은 핵심 디자인 패턴을 적용하고,
**StaticMesh, Material, Texture** 개념을 유기적으로 통합하여,
앞으로 **더 복잡하고 사실적인 씬**을 구성할 수 있는 토대를 마련했습니다.

이번 경험은 단순 구현을 넘어, **게임 엔진이 데이터를 처리하고 세상을 구축하는 원리**에 대한 깊은 이해를 제공했습니다.
